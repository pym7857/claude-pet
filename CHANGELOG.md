...
- **Hover panel highlights the waiting project in red.** When the pet is in surprised state, hovering it now shows the matching tracked-project line with a red left border, bold red text, and a `⚠️` prefix — so you can tell *which* of your open Claude Code sessions is asking for input without alt-tabbing through windows. Sub-paths of a tracked project (e.g. `test-ai-agent/rag_engine` when only `/Users/.../test-ai-agent/` is registered) match by `startsWith` so the parent project gets highlighted.
- **macOS native notification when a tracked session starts waiting for YES/NO.** When a session enters `waitingForUser=true` and survives the 1.5 s debounce, a one-shot system notification fires with the last two path segments of the session's `cwd` (e.g. *"test-ai-agent/rag_engine"*). Useful when you're working in another Claude Code window and don't see the red tray icon. Each new SET transition gets at most one notification, and a per-session 30 s throttle prevents spam when several prompts fire in quick succession.
